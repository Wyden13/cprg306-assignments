    ...
    export default function ItemList({ items, onSelectItem}) {
        ...
        return (
            <div>
                    ...
                    <button 
                        onClick={() => setGroupByCategory(groupByCategory => !groupByCategory)} 
                        className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${
                            groupByCategory 
                                ? "bg-pink-600 text-white shadow-lg shadow-pink-600/50" 
                                : "bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700"
                ...
                {groupByCategory ? (
                    <div className="space-y-6">
                        {Object.entries(handleGroupByCategory(items)).map(([category, items]) => (
                            <div key={category}>
                                <h3 className="text-lg font-bold mb-3 capitalize text-pink-400 border-l-4 border-pink-600 pl-3">{category}</h3>
